...
它的输出不是最终答案，而是候选计划草案。该模块只负责可执行路径召回，不负责最终排序与替换决策。
...
4. 仅基于 `can_feed` 关系，根据输入缺口反向检索上游 Skill。
5. 形成候选计划草案。
...
其中 `CandidatePlan[]` 允许携带排序阶段可用的上下文（例如每个 step 的 slot 合同信息），但不在本模块做替换求解。
...
6. 可执行路径扩展只允许使用 `can_feed` 边，确保候选计划的 I/O 可闭合性可验证。
