...
> **图例说明**
> - 🔵 **蓝色圆角矩形** — 流程（对数据的处理动作）：`表征抽取`、`图谱构建`、`编排与剪枝`、`重排序 / 剪枝`
> - 🟡 **橙黄色双竖线矩形** — 产物（流程输出的结构化数据）：`representations.json`、`skill_graph.json`、`skill_index.json`、`候选编排计划`、`推荐计划`
> - ⬜ **灰色斜角矩形** — 输入源（外部进入管线的原始数据）：`Skill 文件夹 / SKILL.md`、`用户任务`
...
SkillMash 的架构遵循以下核心原则：

**1. 确定性优先，LLM 辅助验证**

优先使用确定性规则处理数据，LLM 输出必须经过解析、规范化与验证才能进入系统状态。例如，图谱构建先用确定性规则生成候选关系（减少 LLM 调用规模），再用 LLM 判断关系类型和置信度；重排序器只排序已有候选计划，不能创造新路径。

**2. Dataclass 合约稳定性**

`SkillRepresentation`、`SkillGraph`、`OrchestrationPlan` 等核心数据模型是跨模块的稳定合约。变更这些合约需要聚焦测试和产物写入/读取的同步更新。

**3. 下游消费规范化表征，不回溯原始数据**

下游图构建和在线编排都消费 `SkillRepresentation`，不再回头读取 `SKILL.md`。这保证了数据流转的单向性和可追踪性。
...
这避免了把"数据是什么"和"数据用什么格式承载"混在一起，使 `can_feed` 关系判断更精确。

**5. 失败时标识输入缺口，而非仅返回失败**

计划即使不完整，也会标识：缺哪个输入、缺口属于哪个 Skill、缺口的 name 和 type、计划是否结构有效但输入不完整。这保留了"失败时标识缺口、提供追问线索"的原则。

**6. 模块边界清晰，产物文件作为阶段接口**

每个阶段通过产物文件（JSON）与下游交互，而非直接调用上游模块的内部函数。这使得各阶段可独立运行、独立调试、独立迭代。
...
`OpenAICompatibleSchemaExtractor` 通过 LLM 从 SKILL.md 文本中提取结构化的输入输出 schema。它将 `RawSkillManifest` 的 frontmatter 与 body（截断至 12000 字符）作为上下文，通过 OpenAI-compatible chat 接口请求 LLM 只返回 JSON，再将响应解析为 `ExtractedSkillSchema`。
...
| **序列化/反序列化** | `save()` / `load()` 支持 JSON 持久化，词汇表状态可跨运行保留 |
...
**`OpenAICompatibleIONameResolver`：LLM 语义判断**

`OpenAICompatibleIONameResolver` 通过 LLM 判断新名称与词汇表已有 term 的语义等价关系，决定归一策略：
...
每一步规范化决策都记录在 `NormalizationDecision` 中，写入 `normalization_decisions.json`：
...
| `raw_value` | LLM 原始值 |
| `token` | 归一化后的 token |
| `normalized_value` | 最终规范化值 |
| `method` | 使用的策略（vocab_exact / vocab_alias / alias_map / create_new / merge_existing / exclude_non_runtime / default / unknown） |
| `confidence` | 决策置信度 |
| `reason` | LLM 给出的理由（仅 LLM Resolver 有） |

这使得每一条规范化结果都有据可查——当下游发现 `query` 被归一为某个 term 时，可以追溯到原始的 "Query or Arxiv ID" 以及 LLM 判断其为 `alias_existing` 的理由。
...
**规范化决策记录（`normalization_decisions.json` 片段）**
...
[
  {
    ...
    "direction": "input",
    "field": "name",
    "raw_value": "Query or Arxiv ID",
    "token": "query_or_arxiv_id",
    "normalized_value": "query",
    "method": "alias_existing",
    "vocab": "io_name_vocab",
    "confidence": 0.95,
    "reason": "search query is the same semantic role as query"
  },
  {
    "skill_id": "aris-arxiv",
    "direction": "output",
    ...
    "vocab": "io_name_vocab",
    "confidence": 0.90,
    "reason": "downloaded PDF is a paper artifact, distinct from query"
  },
  {
    "skill_id": "aris-arxiv",
    "direction": "output",
    "field": "name",
    "raw_value": "Paper Summary",
    "token": "paper_summary",
    "normalized_value": "summary",
    "method": "alias_existing",
    "vocab": "io_name_vocab",
    "confidence": 0.92,
    "reason": "paper summary is a summary of a paper"
  }
]
```
