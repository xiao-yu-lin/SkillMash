...
| 扩展 | 说明 |
| --- | --- |
| **`IONameVocabTerm`** | 继承 `BaseVocabTerm`，用于 I/O 名称规范化 |
...
| **`create_new`** | 新名称是真正新的运行时语义角色，且词汇表未满 | 创建新 term | `transcript` → new term `transcript` |
| **`merge_existing`** | 词汇表已满，无法创建新 term | 强制归入语义最接近的已有 term | 新名 `outline` → merge into `summary` |
| **`exclude_non_runtime`** | 名称仅用于日志/统计/遥测 | 丢弃，不进入词汇表和最终表征 | `analytics_count` → excluded |
